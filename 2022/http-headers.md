...
| Cache    | `Expires`                           | Date/time after which response will be stale. Ignored if `Cache-Control` with `max-age` is present   |
...
| `Proxy-Authorization`               |
| `Age`                               |
| `Cache-Control`                     | Example directives: `max-age`, `no-cache`, `no-store`, `stale-if-error`         |
| `Clear-Site-Data`                   |
| `Expires`                           | An HTTP-date, e.g. `Wed, 21 Oct 2015 07:28:00 GMT`                              |
| `Pragma`                            |
| `Last-Modified`                     |
