...
Before we start, I want to set some expectations. I realize there are a lot of very smart people on this call with long and diverse set of experiences. A difficulty for giving a talk like this is to calibrate the level of sophistication such that it’s not too advanced for an introductory session but also not too trivial to make better use of time. I’ve decided to take the approach where I will start with the lowest-common-denominator and then hopefully quickly build on top of that to arrive at somewhat intermediary subjects to be useful to most. If you’re watching this from a recording, please feel free to fast-forward as you need.

### Disclaimer

I’m not a terminal or Bash scripting expert. I’ve decided to give this talk because I had to write a number of scripts and learned quite a few things in the process, and I want to spread the knowledge of what I now know. Bash and scripting is a huge world, and it’s not very user-friendly. It’s not always clear what is going on (or not). This technology is decades old, but please keep in mind that it is also very powerful. 
...
One of the things I had to learn quickly was how to use the command console. On Windows, it’s called “Command Prompt”, and on the Mac it’s called “Terminal”. These are applications which run a “shell”, which then allow you to submit commands to the operating system or run other applications, typically via their CLI. **CLI** stands for Command-Line Interface - it’s basically a set of commands and parameters an application understands.
...
To launch terminal on the mac, you can go to Launchpad and search for “terminal”. A quicker way is to activate Spotlight using CMD+Space and type in terminal.app.

The built in terminal is pretty bare-bones, so I recommend that you install a better terminal: iTerm2. As with most things, getting started is the hard part. To download iTerm2, you should first install `brew` which is “The Missing Package Manager for macOS”.

Launch terminal. Terminal runs bash as its default shell.
...
Bash recognizes the special $(...) syntax as another bash command, which will run first before its result is passed to the bash command.  This command calls the `curl` executable, with the first argument `-fsSL` and second argument `https://raw.githubusercontent.com/Homebrew/install/master/install.sh`
...
When the shell is not running an application, it waits for commands in what’s called the “REPL” mode. This stands for Read, Eval(uate), Print loop. When the shell is in this mode, you can put in new commands and edit the current command as if in a single-line text editor.  
...
You can see the previous commands you entered using the `**history**`** **command**.**
Use `CMD+T` to create a new shell tab, or `CMD+N` to create a new terminal window. Use the **`exit`** command to make the current tab go away.

Finally, some commands support auto-completion which can be activated by pressing `tab`. If you’re in the middle of typing in the name of a file where the shell can access it, pressing `tab` will auto-complete it on the first try, unless there are multiple matches. In this case nothing will happen on the first try but a subsequent pressing of the `tab` will show the matching entries without losing your command. (Note: This behavior is shell/configuration specific.)


## iTerm2 Profiles

Profiles allow you to specify options for the command shell. One of my favorite features of iTerm2 is “Triggers“.  
...
[Image: image.png]Here are a few useful regular expressions: 
Paths: `(/?[\w\.\-~]+/)+([\w\.\-~]+)*`
Filenames: `(?>([\w\!-]*\.)+[\w\!-]+)(?!\/)`
URLs: `[A-Za-z]+://\S+`
...
Let’s tokenize the arguments to the executable. There are 7 tokens. How the executable interprets these tokens is completely proprietary, but there are conventions:

Anything that follows a single dash is either a flag or an option indicator. A “flag” changes or overrides the default operation of the executable. An “option indicator” indicates that the subsequent token is a value for the option.
...
There are some other patterns that you might see:

If you’re a core developer, you will have noticed that BLT uses the following pattern, which is the opposite of what I call the standard:

```
blt --build compile post
```

In this case, the command is “build” and arguments to build is compile and post and also ordering matters.

If you’re a JAVA developer, in Maven, you have the -Kvalue pattern:
...
Many CLI commands support a built-in help function. You can invoke it with one ore more of the following: `-?` or `-h` or` --help`. 

Some CLI commands, when run without any arguments will display its usage. (Be careful though, if there are no arguments, some CLI commands will switch into a mode where it expects you to provide input using the keyboard. You can cancel out by pressing CTRL+C.)
...
`find` command recursively looks for files whose name matches the GLOB expression, outputs their full-path to STDOUT, which gets piped into `xargs`. For each line it receives, `xargs` converts the line into an argument and passes it to `rm` as an argument, thereby deleting the file. 
...
To set a variable, simply do `VARIABLE_NAME=VALUE`, paying attention to the fact that there are _no spaces before and after the equals sign_.


```
o>MY_VAR="Hello World"
o>echo $MY_VAR
Hello World
```
...
The export command means that the new value should remain available outside of the scope where the variable is defined.
...
Pressing any of the keyboard shortcuts in **bold-face**, listed above will activate the assigned function. For example `f2` will toggle the preview using the configured viewer. 
...
It’s common to pipe output into grep, which can be convenient. For example, if you can’t remember the alias for a command, you can do:
...
`tail` can also be used to skip the first few lines using the** `+`** prefix before the line. The number is the line number to start from, not the number of lines to skip. In other words, if you want to skip the first lune, specify `+2`. Tail doesn’t accept a `+` and `-` argument at the same time, but this can be achieved via piping.
...
**TIP FOR CORE DEVELOPERS**
A useful thing to do is to monitor only specified log record types as the app is running. The `-f` argument tells tail to keep monitoring the file and output newly added lines to its STDOUT.

```
alias tw='tail -f ~/blt/app/main/core/sfdc/logs/sfdc/output.log | grep -E "(aprst)|(aiuim)|(ailtn)"'
```



### Some other useful commands to know about are as follows:
...
`**awk**`** **is another text manipulating tool that’s beyond the scope of this document.


### Querying JSON

From its **website**: “jq is a lightweight and flexible command-line JSON processor”
...
Learning the jq expressions is a good investment if you need to extract values from CLIs that output in JSON.
...
**Question**: Given the STATS variable as defined below, output the median value, ideally with a single-line command input.
...
alias m2360='export M2_HOME=/Users/loz/sdk/apache-maven-3.6.0'

echo 'To select an environment, run "se".'
function se() {
    select ENV in Aura Bazel BLT Java
    do
        clear
        case $ENV in
            Aura)
                echo 'Environment is now Aura'
                jh212
                m2360
                cd ~/git/aura/aura
                break
                ;;
            Bazel)
                echo 'Environment is now Bazel'
                cd ~/git/services/sfdc-bazel
                source ./venv.sh
                break
                ;;
            BLT)
                echo 'Environment is now BLT'
                source ~/blt/env.sh
